...
final class AVImageTests: XCTestCase {
    ...
    let width = 1080
    let height = 1920
    let pixelFormat = PixelFormat.YUV420P
    var image: Image!

    override func setUp() {
        super.setUp()

        image = Image(width: width, height: height, pixelFormat: pixelFormat, align: 1)
    }
    ...
    func testBuf() {
        ...
        try! Image.fillPointers(&data, pixelFormat: pixelFormat, height: height, buffer: image.data[0], linesizes: linesizes)
    ...
    func testLinesizes() {
        ...
        let linesizes1 = (0..<4).map({ try! Image.getLinesize(pixelFormat: pixelFormat, width: width, plane: $0) })
        XCTAssertEqual(Array(image.linesizes), linesizes1.map({ Int32($0) }))

        var linesizes2 = [Int32](repeating: 0, count: 4)
        try! Image.fillLinesizes(&linesizes2, pixelFormat: pixelFormat, width: width)
        XCTAssertEqual(Array(image.linesizes), linesizes2)
    }

    func testTotalSize() {
        let size = try! Image.getBufferSize(pixelFormat: pixelFormat, width: width, height: height)
