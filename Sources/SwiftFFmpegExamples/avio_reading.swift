...
private func read_packet(
  opaque: UnsafeMutableRawPointer?, buffer: UnsafeMutablePointer<UInt8>?, size: Int
) -> Int {
  ...
  print(
    "ptr: 0x\(String(Int(bitPattern: bdPtr.pointee.ptr), radix: 16, uppercase: false))  size: \(bdPtr.pointee.size)"
  )
