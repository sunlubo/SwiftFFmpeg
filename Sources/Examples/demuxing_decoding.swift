...
private func decode_video_packet(
    ...
    while true {
        ...
        if frame.width != image.width || frame.height != image.height || frame.pixelFormat != image.pixelFormat {
            fatalError("""
...
func demuxing_decoding() throws {
    if CommandLine.argc < 4 {
        print("Usage: \(CommandLine.arguments[0]) \(CommandLine.arguments[1]) input_file video_output_file audio_output_file")
        return
    }
    ...
    while let _ = try? fmtCtx.readFrame(into: pkt) {
        if pkt.streamIndex == videoIndex {
            try decode_video_packet(codecCtx: videoCodecCtx, pkt: pkt, frame: frame, image: image, file: videoOutputFile)
        } else if pkt.streamIndex == audioIndex {
            try decode_audio_packet(codecCtx: audioCodecCtx, pkt: pkt, frame: frame, file: audioOutputFile)
        }
        pkt.unref()
    }

    // flush cached frames
    try decode_video_packet(codecCtx: videoCodecCtx, pkt: nil, frame: frame, image: image, file: videoOutputFile)
    ...
    print("ffplay -f rawvideo -pix_fmt \(videoCodecCtx.pixelFormat) -video_size \(videoCodecCtx.width)x\(videoCodecCtx.height) \(videoOutput)")
