...
private func fill_yuv_image(image: AVImage, frameIndex: Int) {
    ...
    for y in 0..<image.height {
        for x in 0..<image.width {
            image.data[0]![y * Int(image.linesizes[0]) + x] = UInt8(truncatingIfNeeded: x + y + frameIndex * 3)
        }
    }
    ...
    for y in 0..<image.height / 2 {
        for x in 0..<image.width / 2 {
            image.data[1]![y * Int(image.linesizes[1]) + x] = UInt8(truncatingIfNeeded: 128 + y + frameIndex * 2)
            image.data[2]![y * Int(image.linesizes[2]) + x] = UInt8(truncatingIfNeeded: 64  + x + frameIndex * 5)
        }
    }
}
...
func scaling_video() throws {
    ...
    guard let swsCtx = SwsContext(
        ...
        fatalError("Impossible to create scale context for the conversion fmt:\(srcPixFmt.name) s:\(srcW)x\(srcH) -> fmt:\(dstPixFmt.name) s:\(dstW)x\(dstH)")
    }
